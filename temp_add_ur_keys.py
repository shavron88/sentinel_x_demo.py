...
# Find the UR dict closing - it ends with:
#     'tech.restapi.role': 'انٹیگریشن',
#   };
# Find the line with 'tech.restapi.role' and insert after it

pattern = r"(    'tech\.restapi\.role':\s*'[^']+',\n)(\s*\};)"
match = re.search(pattern, content)
if match:
    insert_pos = match.start(2)  # Before "  };"
    new_entries = "\n"
    ...
    for line_num, line in enumerate(content.split('\n')):
        if 'tech.restapi.role' in line:
            print(f"Found at line {line_num+1}: {repr(line)}")
